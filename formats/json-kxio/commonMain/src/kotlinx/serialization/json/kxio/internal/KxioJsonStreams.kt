    ...
    override fun writeQuoted(text: String) {
        ...
        for (i in text.indices) {
            ...
            if (c < ESCAPE_STRINGS.size && ESCAPE_STRINGS[c] != null) {
                sink.writeString(text, lastPos, i) // flush prev
                sink.writeString(ESCAPE_STRINGS[c]!!)
                lastPos = i + 1
            }
        }

        if (lastPos != 0) sink.writeString(text, lastPos, text.length)
        else sink.writeString(text)
        sink.writeCodePointValue(QUOTE_CODE)
    }
...
internal class KxioSerialReader(private val source: Source): InternalJsonReader {
    /*
